...
- [ ] T003 [P] Initialize frontend Next.js App Router project in `frontend/` with TypeScript strict, Tailwind CSS, TanStack Query v5
- [ ] T004 [P] Configure backend lint/format/typecheck in `backend/pyproject.toml` (Ruff, mypy)
...
- [ ] T047 [P] [US2] Contract test TurnResponse triage fields in `backend/tests/contract/test_turn_response_schema.py`
...
- [ ] T053 [P] [US3] Unit test session store turn append/trim in `backend/tests/unit/memory/test_session_store.py`
...
- [ ] T057 [US3] Implement turn trimming (max 20) in `backend/app/memory/session_store.py`
